...
typedef NS_ENUM(NSInteger, JCNetworkStatus) {
    kNetworkStatusUnknown          = -1,//未知网络
    kNetworkStatusNotReachable     = 0,//网络无连接
    kNetworkStatusReachableViaWWAN = 1,//2，3，4G网络
    kNetworkStatusReachableViaWWAN2G = 2,//2G网络
    kNetworkStatusReachableViaWWAN3G = 3,//3G网络
    kNetworkStatusReachableViaWWAN4G = 4,//4G网络

    kNetworkStatusReachableViaWiFi = 9,//WIFI网络
};
...
typedef void(^ResponseFailWithCode)(NSError *error, NSInteger statusCode);

// 清空缓存是否成功
typedef void(^ActionSuccess)(void);
typedef void(^ActionFail)(NSError *error);
 ...
 *
 *    @param data                文件nsdata数据
 *    @param url                   开发者服务器 url
 *    @param fileName        文件名
 *    @param name                文件对应的 key , 开发者在服务器端通过这个 key 可以获取到文件二进制内容
 *    @param header            http头部信息
